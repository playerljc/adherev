...
const SearchTable = extend({
    ...
    methods: {
        ...
        fixedHeaderAutoTableEffectLayout(prevState) {
            if (this.fixedHeaderAutoTable) {
                ...
                if (dataSource &&
                    ...
                    const tableWrapRef = this.$refs.tableWrapRef;
                    ...
                    const tableHeaderHeight = tableWrapRef.querySelector('.ant-table-thead')?.offsetHeight || 0;
                    const tablePaginationHeight = tableWrapRef.querySelector('.ant-table-pagination')?.offsetHeight || 0;
                    this.scrollY =
        ...
        onCellConfigReducers(params) {
            ...
            return this.$data.$cellConfigReducers.reduce((params, reducer) => {
        ...
        onRowConfigReducers(params) {
            ...
            return this.$data.$rowConfigReducers.reduce((params, reducer) => {
        ...
        getTableColumns(h) {
            ...
            const columns = this.getColumns()
                ...
                const loop = (_column) => {
                    ...
                    if ('$resizable' in _column && !!_column?.$resizable) {
                        _res = this.$data.$columnResizable.searchTableResizableColumnItem(this, index, _column);
                    }
                    // @ts-ignore
            ...
            if (isShowNumber) {
                const numberGeneratorRule = this.getNumberGeneratorRule() || SearchTable.NUMBER_GENERATOR_RULE_ALONE;
                ...
                return [
                    {
                        ...
                        ...{
                            ...
                            customRender: (text, row, index) => {
                                ...
                                //
                                // const { page = 0, limit = 10 } = this;
                                return (<ConditionalRender conditional={numberGeneratorRule === NUMBER_GENERATOR_RULE_ALONE}>
                    <span slot="default">
        ...
        getScrollHeaderEl() {
            const tableWrapRef = this.$refs.tableWrapRef;
            return tableWrapRef?.querySelector('.ant-table-wrapper > .ant-spin-nested-loading > .ant-spin-container > .ant-table > .ant-table-content > .ant-table-scroll > .ant-table-header');
        },
        getScrollBodyEl() {
            const tableWrapRef = this.$refs.tableWrapRef;
            return tableWrapRef?.querySelector('.ant-table-wrapper > .ant-spin-nested-loading > .ant-spin-container > .ant-table > .ant-table-content > .ant-table-scroll > .ant-table-body');
        },
        /**
         ...
         * @param params
         */
        renderTableNumberColumn(h, number = '', params) {
            return (this.$scopedSlots?.tableNumberColumn?.({ context: this.getContext?.()?.context, number }) || (<span>{number}</span>));
        },
        /**
         * renderColumnSetting
         * @description 创建列设置组件
        ...
        renderColumnSetting(h) {
            ...
            return (this.$scopedSlots?.columnSetting?.({ context: this.getContext?.()?.context, columns }) || (
            // @ts-ignore
            <ColumnSetting columns={columns} onShowColumns={(checked) => {
         ...
         * @description 表格密度设置
         */
        renderTableDensitySetting(h) {
            return (this.$scopedSlots?.tableDensitySetting?.(this) || (
            // @ts-ignore
            <TableDensitySetting density={this.tableDensity} onChange={(density) => {
         ...
         * @param h
         */
        renderSearchFooter(h) {
            const { isShowExpandSearch } = this;
            const defaultItems = [
                <Button 
                // @ts-ignore
                class={`${selectorPrefix}-searchfooteritem`} type="primary" key="search" onClick={() => {
                        this.page = 1;
                        this.$nextTick(function () {
                            this.onSearch();
                        });
                    }}>
          <i class={classNames(`${selectorPrefix}-searchfooteritem-search-btn-icon`, 'iconfont iconsousuo')}/>
          {Intl.tv('查询')}
        </Button>,
                <Button 
                // @ts-ignore
                class={`${selectorPrefix}-searchfooteritem`} key="reset" onClick={this.onClear}>
          {Intl.tv('重置')}
        </Button>,
            ];
            if (isShowExpandSearch) {
                defaultItems.push(<ConditionalRender conditional={this.expand}>
            <a slot="noMatch" key="expand" class={`${selectorPrefix}-searchfooteritem-expand-search-up-btn`} 
            ...
            <a key="hide" class={`${selectorPrefix}-searchfooteritem-expand-search-down-btn`} 
                // style="display: flex; align-items: center"
                onClick={() => {
                        // this.expand = false;
                        this.onSearchPanelCollapseBefore && this.onSearchPanelCollapseBefore();
            ...
            return (<div class={`${selectorPrefix}-searchfooterwrapper`}>
          {this.$scopedSlots?.searchFooter?.({ context: this.getContext?.()?.context, items }) ||
                    items.map((t, index) => (<div key={index} class={`${selectorPrefix}-searchfooteritem`}>
                {t}
              </div>))}
        </div>);
        },
        ...
        renderInner(h) {
            ...
            const { className, wrapStyle, tableClassName, tableStyle, searchStyle, searchClassName, fitSearch, fitTable, autoFixed, fixedTableSpaceBetween, } = this;
            return (<FlexLayout direction="vertical" style={wrapStyle || ''} class={classNames(selectorPrefix, fixedTableSpaceBetween ? 'fixedtablespacebetween' : '', className || '')}>
          <Fixed class={classNames(`${selectorPrefix}-searchwrapper`, searchClassName || '')} style={searchStyle || ''} fit={fitSearch}>
            <FlexLayout direction="vertical">
              <ConditionalRender conditional={this.$scopedSlots.searchFormBefore || !!this.renderSearchFormBefore}>
                <Fixed slot="default">
              ...
              <ConditionalRender conditional={this.$scopedSlots.searchFormAfter || !!this.renderSearchFormAfter}>
                <Fixed slot="default">
          ...
          <ConditionalRender conditional={!!this.$scopedSlots.tableHeader || !!this.renderTableHeader}>
            <Fixed slot="default">
              {this.$scopedSlots?.tableHeader?.(this) || this.renderTableHeader(h)}
            </Fixed>
          </ConditionalRender>

          <Auto class={classNames(`${selectorPrefix}-autowrapper`, tableClassName || '', autoFixed ? 'autofixed' : '')} style={tableStyle || ''} fit={fitTable} autoFixed={autoFixed}>
            <div ref="tableWrapRef" class={`${selectorPrefix}-tablewrapper`}>
              {this.renderTable(h)}
            </div>
          </Auto>

          <ConditionalRender conditional={!!this.$scopedSlots.tableFooter || !!this.renderTableFooter}>
            <Fixed slot="default">
         ...
         */
        renderSearchTable(h) {
            return (this.$scopedSlots?.searchTable?.(this) || (<div class={`${selectorPrefix}-wrap`}>{this.$renderSuspense(h)}</div>));
        },
    },
    render(h) {
