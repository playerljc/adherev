...
const { FormItemGeneratorToDict, AntFormItemNormalize: { DatePicker, InputNumberDecimal1, InputNumberDecimal2, InputNumberInteger, RangePicker, TimePicker, }, } = AntdFormItem;
 ...
 * @constructor
 */
export const ProSearchTable = (serviceName) => extend({
    className: 'ProSearchTable',
    ...
    created() {
        ...
        if (!('openSearchParamsMemory' in this) ||
    ...
    methods: {
        ...
        unMountSearchAndPaginParamsDeal() {
            ...
            if (TableImplementSearchAndvPaginParamsMemo.isEmpty()) {
                TableImplementSearchAndvPaginParamsMemo.add(pathname, {
                    [componentId]: {
                        search: searchParams,
                        page: this.page,
                        limit: this.limit,
                ...
                const item = TableImplementSearchAndvPaginParamsMemo.findByPath(pathname); // TableImplementSearchAndvPaginParamsMemo.find((t) => t.path === pathname);
                if (item) {
         ...
         */
        initSearchAndPaginParams(queryReduce) {
            ...
            const item = TableImplementSearchAndvPaginParamsMemo.findByPath(this.$data.$pathname);
            const componentId = this.getComponentId();
        ...
        getParams() {
            ...
            const loop = (columns) => {
                columns.reduce((params, column) => {
                    ...
                    if ([this.getOptionsColumnDataIndex(), this.getLinkColumnDataIndex(), '_number'].includes(dataIndex)) {
                        return params;
                    }
                    if (searchConfig.type === 'rangePicker') {
                        if (searchConfig.startName)
                            params[searchConfig.startName] = null;
                        if (searchConfig.endName)
                            params[searchConfig.endName] = null;
                    }
        ...
        getDateState(state) {
            // null | null字符串 | 时间字符串
            const dateKeys = Object.keys(state).filter((key) => state[key] === null ||
                state[key] === 'null' ||
        ...
        getFetchDataParams() {
            ...
            const rangeDateKeys = Object.keys(this.searchParams).filter((key) => ['start', 'Start'].some((t) => key.indexOf(t) !== -1) ||
                ['end', 'End'].some((t) => key.indexOf(t) !== -1));
            rangeDateKeys.forEach((key) => {
                dateSearchParams[key] = this.searchParams[key]
                    ? `${this.searchParams[key].format(Resource.Dict.value.ResourceMomentFormat10.value())} ${['start', 'Start'].some((t) => key.indexOf(t) !== -1)
                        ? '00:00:00'
                        : ['end', 'End'].some((t) => key.indexOf(t) !== -1)
                            ? '23:59:59'
                            : ''}`.trim()
            ...
            const dateKeys = Object.keys(this.searchParams).filter((key) => !(key in dateSearchParams) && this.searchParams[key] instanceof moment);
            dateKeys.forEach((key) => {
                dateSearchParams[key] = this.searchParams[key]
        ...
        getTableColumns(h) {
            ...
            if (this.hasOptionColumnFixed()) {
                const options = columns.find((t) => t.dataIndex === (this.getOptionsColumnDataIndex() || '_options'));
            ...
            const linkColumn = columns.find((t) => t.dataIndex === (this.getLinkColumnDataIndex() || '_linkColumn'));
            if (linkColumn) {
            ...
            const res = columns
                ...
                const loop = (t) => {
                    ...
                    if (!['_number', this.getOptionsColumnDataIndex()].includes(dataIndex) &&
                        showColumnHeader) {
                        column = {
                            ...column,
                            scopedSlots: {
                ...
                columns
                    ...
                    searchFormGroupData.push({
                      ...
                      {this.renderGridSearchFormGroupDataItem(type, {
                            ...
                            noMatch: $search.renderNoAuthority ? (<Value {...($search.valueAttrs || {})}>
        ...
        getScopedSlots(h) {
            return {
                ...
                [this.getFilterDropdownSlot()]: ({ confirm, column }) => {
                    ...
                    return (<div class={`${_selectorPrefix}-headersearchwrap`}>
                ...
                <div class={`${_selectorPrefix}-headersearchwrap-footer`}>
                  <Button size="small" onClick={() => {
                            if (type === 'rangePicker') {
                                if (searchConfig.startName)
                                    this[searchConfig.startName] = null;
                                if (searchConfig.endName)
                                    this[searchConfig.endName] = null;
                            }
                            else {
                  ...
                  <Button size="small" type="primary" onClick={() => this.onSearch().then(() => confirm())}>
                    {Intl.tv('确定')}
                  </Button>
                </div>
              </div>);
        ...
        renderSearchForm(h /*columnCount, data*/) {
            return this.renderGridSearchFormGroup(h, ...this.getGridSearchFormGroupParams( /*columnCount, data*/));
        },
        /***
        ...
        renderSearchFooterItems(h, _defaultItems) {
            ...
            if (this.$data.$hasAdvancedSearchPanel && this.expand) {
                const SearchButtonComponent = (<Button slot="default" icon="filter" type="primary" onClick={() => {
                        this.advancedSearchPanelCollapse = true;
                    }}>
              {Intl.v('高级搜索')}
            </Button>);
                if (this.$data.$advancedSearchConfig.insertSearchButton) {
                    this.$data.$advancedSearchConfig.insertSearchButton(defaultItems, SearchButtonComponent);
                }
                else {
                    const resetIndex = defaultItems.findIndex((t) => 'tag' in t &&
                        t.tag &&
            ...
            return this.renderSearchFooterItemsImpl(h, defaultItems).map((t) => 'tag' in t && t.tag && t.tag.startsWith('vue-component-') ? t : t.value);
        },
        /**
        ...
        renderGridSearchFormGroupDataItem(type, { searchConfig, column, dataIndex }) {
            const renderInput = ({ searchConfig, dataIndex }) => {
                return (<Input value={this[dataIndex]} onChange={(e) => this.onInputChange(dataIndex, e)} {...{
                    props: {
                        ...(searchConfig.props || {}),
                    },
                }}/>);
            ...
            const renderTextArea = ({ searchConfig, dataIndex }) => {
                return (<Input.TextArea value={this[dataIndex]} onChange={(e) => this.onInputChange(dataIndex, e)} {...{
                    props: {
                        ...(searchConfig.props || {}),
                    },
                }}/>);
            ...
            const renderInputNumber = ({ searchConfig, dataIndex }) => {
                return (<InputNumber value={this[dataIndex]} onChange={(e) => this.onSelectChange(dataIndex, e)} {...{
                    props: {
                        ...(searchConfig.props || {}),
                    },
                }}/>);
            ...
            const renderInputNumberDecimal1 = ({ searchConfig, dataIndex }) => {
                return (<InputNumberDecimal1 value={this[dataIndex]} onChange={(e) => this.onSelectChange(dataIndex, e)} {...{
                    props: {
                        ...(searchConfig.props || {}),
                    },
                }}/>);
            ...
            const renderInputNumberDecimal2 = ({ searchConfig, dataIndex }) => {
                return (<InputNumberDecimal2 value={this[dataIndex]} onChange={(e) => this.onSelectChange(dataIndex, e)} {...{
                    props: {
                        ...(searchConfig.props || {}),
                    },
                }}/>);
            ...
            const renderInputNumberInteger = ({ searchConfig, dataIndex }) => {
                return (<InputNumberInteger value={this[dataIndex]} onChange={(e) => this.onSelectChange(dataIndex, e)} {...{
                    props: {
                        ...(searchConfig.props || {}),
                    },
                }}/>);
            ...
            const renderSelect = ({ searchConfig, dataIndex }) => {
                ...
                return (<Component value={this[dataIndex]} onChange={(e) => this.onSelectChange(dataIndex, e)} {...{
                    props: {
                        ...(searchConfig.props || {}),
                    },
                }}/>);
            ...
            const renderRadioCustom = ({ searchConfig, dataIndex }) => {
              ...
              {(data) => this.$scopedSlots?.[searchConfig?.renderChildren]?.(data) ||
            ...
            const renderCheckBoxCheckAllHorizontal = ({ searchConfig, dataIndex }) => {
                const Component = FormItemGeneratorToDict[`${searchConfig.dictName}CheckAllHorizontalFormItem`];
                return (<Component value={this[dataIndex]} onChange={(e) => this.onSelectChange(dataIndex, e)} {...{
                    props: {
                        ...(searchConfig.props || {}),
                    },
            ...
            const renderCheckBoxCustom = ({ searchConfig, dataIndex }) => {
              ...
              {(dataSource) => this.$scopedSlots?.[searchConfig?.renderChildren]?.(dataSource) ||
            ...
            const renderTablePagingSelect = ({ searchConfig, dataIndex }) => {
                const Component = FormItemGeneratorToDict[`${searchConfig.dictName}PaginationSelectFormItem`];
                return (<Component value={this[dataIndex]} onChange={(e) => this.onSelectChange(dataIndex, e)} {...{
                    props: {
                        ...(searchConfig.props || {}),
                    },
            ...
            const renderTablePagingMultiSelect = ({ searchConfig, dataIndex }) => {
                const Component = FormItemGeneratorToDict[`${searchConfig.dictName}PaginationMulitSelectFormItem`];
                return (<Component value={this[dataIndex]} onChange={(e) => this.onSelectChange(dataIndex, e)} {...{
                    props: {
                        ...(searchConfig.props || {}),
                    },
            ...
            const renderDatePicker = ({ searchConfig, dataIndex }) => {
                return (<DatePicker value={this[dataIndex]} onChange={(moment) => {
                        this.setState({
            ...
            const renderTimePicker = ({ searchConfig, dataIndex }) => {
                return (<TimePicker value={this[dataIndex]} onChange={(moment) => {
                        this.setState({
                            [dataIndex]: moment ? moment : null,
                        });
                    }} {...{
            ...
            const renderRangePicker = ({ searchConfig }) => {
                ...
                return (<RangePicker value={[this[startName], this[endName]]} onChange={(moments) => {
            ...
            const renderSlider = ({ searchConfig, dataIndex }) => {
                return (<Slider value={this[dataIndex]} onChange={(e) => this.onSelectChange(dataIndex, e)} {...{
                    props: {
                        ...(searchConfig.props || {}),
                    },
                }}/>);
            ...
            const renderSliderRange = ({ searchConfig }) => {
                return (<Slider range value={this[dataIndex]} onChange={(e) => this.onSelectChange(dataIndex, e)} {...{
                    props: {
                        ...(searchConfig.props || {}),
                    },
                }}/>);
            ...
            const renderRate = ({ searchConfig }) => {
                return (<Rate value={this[dataIndex]} onChange={(e) => this.onSelectChange(dataIndex, e)} {...{
                    props: {
                        ...(searchConfig.props || {}),
                    },
                }}/>);
            ...
            const renderSwitch = ({ searchConfig }) => {
                return (<Switch value={this[dataIndex]} onChange={(e) => this.onSelectChange(dataIndex, e)} {...{
                    props: {
                        ...(searchConfig.props || {}),
                    },
                }}/>);
            ...
            const renderCustom = ({ searchConfig, column, dataIndex }) => {
                return (this.$scopedSlots?.[searchConfig?.render]?.({ searchConfig, column, dataIndex }) ||
        ...
        renderGridSearchFormGroup(h, group, props, advancedSearchConfig) {
            ...
            // 配置
            this.$data.$advancedSearchConfig = merge(this.$data.$advancedSearchConfig, advancedSearchConfig || {});
            // 标准的查询面板
            ...
            if (this.$data.$advancedSearchConfig.rowCount !== 'auto') {
                const { rowCount, detail } = TableGridLayout.getRenderDetail(h, this, group, defaultProps);
                if (rowCount > this.$data.$advancedSearchConfig.rowCount) {
                    // 显示的组数据
                    const gData = [];
                    // 剩余的组数据
                    const remainingGData = [];
                    // 需要的行数
                    ...
                    while (needRowCount !== 0) {
                        ...
                        if (groupDetail.rowCount <= needRowCount) {
                            ...
                            gData.push({
                                ...groupData,
                                data: groupData.data.slice(details[0].startIndex, details[details.length - 1].endIndex + 1),
                            });
                            remainingGData.push({
                                ...groupData,
                    ...
                    return (<div class={`${_selectorPrefix}-gridsearchformgroupwrap`}>
                {renderGridSearchFormGroup(h, this, gData, defaultProps)}
                {/*@ts-ignore*/}
                <AdvancedSearchPanel groupData={group} tableGridLayoutConfig={defaultProps} remainingGroupData={remainingGData} advancedSearchConfig={this.$data.$advancedSearchConfig} searchFn={() => new Promise((resolve) => {
                            this.page = 1;
                            this.$nextTick(function () {
                                this.onSearch().then(() => resolve());
                            });
                        })} resetFn={() => this.onClear()} collapseFn={(collapse) => {
            ...
            this.$data.$hasAdvancedSearchPanel = false;
            return (<div class={`${_selectorPrefix}-gridsearchformgroupwrap`}>{StandardSearchPanel}</div>);
        },
        /**
         * renderOptionColumn
         * @param defaultItems
...
//# sourceMappingURL=ProSearchTable.jsx.map