import { Button, InputNumber, Rate, Slider, Switch } from 'ant-design-vue';
...
const ProTableFactory: (
  ...
  extend({
    ...
    mixins: [superClass],
    props: {
      openSearchParamsMemory: bool().def(true),
    },
    data() {
      return {
    ...
    methods: {
      ...
      getTableColumns() {
        ...
        //   ...t,
        // }));
        const columns = this.$getTableColumnsSearchTable();
              ...
              searchFormGroupData.push({
                ...
                value: ConditionalRender.conditionalRender({
                  ...
                  noMatch: $search.renderNoAuthority ? (
                    <Value {...($search.valueAttrs || {})}>
                      {this.$slots?.[$search?.renderNoAuthority]?.() ||
       ...
       * @override
       */
      renderSearchForm(/*columnCount, data*/) {
      ...
      renderSearchFooterItemsImpl(defaultItems) {
        const tableDensitySetting = this.renderTableDensitySetting();
        const columnSetting = this.renderColumnSetting();
      ...
      renderGridSearchFormGroupDataItem(type, { searchConfig, column, dataIndex }) {
        ...
        const renderRadioCustom = ({ searchConfig, dataIndex }) => {
          ...
          return (
            ...
            >
              {(data) =>
                this.$slots?.[searchConfig?.renderChildren]?.(data) ||
        ...
        const renderCheckBoxCustom = ({ searchConfig, dataIndex }) => {
          ...
          return (
            ...
            >
              {(dataSource) =>
                this.$slots?.[searchConfig?.renderChildren]?.(dataSource) ||
        ...
        const renderCustom = ({ searchConfig, column, dataIndex }) => {
          return (
            this.$slots?.[searchConfig?.render]?.({ searchConfig, column, dataIndex }) ||
