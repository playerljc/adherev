        ...
        if ((status >= 200 && status < 300) || status === 304) {
            ...
            if (contentType.indexOf(Ajax.CONTENT_TYPE_APPLICATION_JSON) !== -1) {
                ...
                if (showWarn && jsonObj[codeKey] !== codeSuccess) {
                    warnInfo(adherev_util_intl_1.default.tv('提示'), jsonObj[messageKey]);
                }
...
function sendPrepare(_a, _b) {
    ...
    ;
    var indicator;
    var defaultLoadingText = adherev_util_intl_1.default.tv('加载中') + "...";
...
function getSendParams(_a) {
    ...
    // application/json
    if (contentType.indexOf(Ajax.CONTENT_TYPE_APPLICATION_JSON) === 0 && adherev_util_1.default.isRef(data)) {
    ...
    // multipart/form-data
    if (contentType.indexOf(Ajax.CONTENT_TYPE_MULTIPART_FORM_DATA) === 0 && adherev_util_1.default.isObject(data)) {
