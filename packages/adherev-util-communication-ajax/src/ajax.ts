...
import { IConfig, ISendArg, ISendPrepareArg, Method } from './types';
  ...
  if (xhr.readyState === Ajax.READY_STATE_DONE) {
    ...
    if ((xhr.status >= 200 && xhr.status < 300) || xhr.status === 304) {
      ...
      // response ContentType是xml
        ...
        resolve(resolveData({ show, data: xhr.responseXML, indicator, xhr }));
      }
...
function deal401(this: Ajax) {
  ...
  window.location.href = (Util as any).casUrl({
...
function deal402(this: Ajax) {
  ...
  window.location.href = (Util as any).casLogoutUrl({
    baseUrl: this.systemManagerBaseURL,
    enterUrl: window.location.href,
    params: '&code=402',
  });
